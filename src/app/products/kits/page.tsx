...
export default function Products() {
    ...
    const [kits, setKits] = useState<FullKit[]>([]);
    ...
    const handleSendCategories = async (updatedCategories: string[]) => {
        ...
        const res = await FilteredKits(filteredData);
                setPageNumber(1);
        setProductsSplit(0);
        setIsLoading(false); // Llama a la función para obtener los kits filtrados
        setKits(res);
        setPricesChanges(false); // Limpia kits antes de actualizar
        //console.log("Enviando: ", filteredData); // Asegúrate de que envías los datos correctos
        // console.log("Recibiendo: ", res); // Imprime los resultados de los kits filtrados
        // setKits(res); // Actualiza el estado con los nuevos kits
        } catch (error) {
    ...
    const handleSplitNext = () => {
        if (kits.length >= productsSplit + 16) {
    ...
    const totalProducts = kits.length;

    // Calculamos x como el número de páginas (o grupos) en base a 16 kits por grupo
    ...
    useEffect(() => {
        ...
        try {
            const res = await getKits(); // Llama a la función para obtener los kits
            setKits(res);
            setIsLoading(false);
